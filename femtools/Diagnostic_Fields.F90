...
module diagnostic_fields
  ...
  use fldebug
  use global_parameters, only:FIELD_NAME_LEN, current_time, OPTION_PATH_LEN, timestep
  use futils
  use spud
  use Vector_Tools
  use parallel_tools
  ...
  subroutine calculate_vector_diagnostic_variable_single_state(state, d_field_name, &
    ...
    select case(d_field_name)
     ...
     case("InitialCoordinate")
        call calculate_initial_coordinate_field(state, d_field)
...
!     end if
   ...
   subroutine calculate_initial_coordinate_field(state, field)
    type(state_type), intent(in) :: state
    type(vector_field), intent(inout) :: field
   ...
   end subroutine calculate_initial_coordinate_field
